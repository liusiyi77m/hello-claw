...
每个文件都是纯 Markdown，直接用文本编辑器编辑即可。下面逐一说明"该写什么"。
...
## 边界
- 不讨论政治、宗教等敏感话题
- 不执行未经确认的破坏性操作（删除文件、force push 等）
- 涉及金钱交易时，必须二次确认
```
...
- 偏好：喜欢简洁的代码，讨厌过度工程
- 时区：Asia/Shanghai
- 工作语言：中文沟通，英文写代码和 commit message
```
...
## GitHub 仓库操作
当收到涉及 GitHub 仓库的任务时：
1. 先用 `gh api repos/{owner}/{repo}` 检查是否有 push 权限
2. 有权限 → `gh repo clone` 到工作区，创建分支直接操作
3. 无权限 → `gh repo fork` → clone fork，完成后向上游提 PR
4. 遇到认证错误，先检查 Token 是否过期或 scope 不足
...
## exec（命令执行）
- 执行破坏性命令前必须确认（rm -rf, git push --force 等）
- 长时间运行的命令加 timeout
...
- 检查工作区 git 状态：`git -C ~/.openclaw/workspace status`
- 如果有未推送的记忆更新，自动 push
```
...
## 项目
- hello-claw：OpenClaw 中文教程项目，VitePress 构建
- my-app：个人 SaaS 项目，Next.js + Supabase
...
## 重要决策
- 2026-03-10：决定用 Claude Code 作为编码引擎，OpenClaw 作为编排层
```

...
这是 OpenClaw 最有工程价值的一层定制接口。它足够轻，不需要你先理解整个 Gateway；又足够强，能够把提示词、工具调用、外部脚本、依赖门禁和 Slash Command 串成一条完整链路。
...
这些原则看起来普通，但它们决定了一个 Skill 到底是“能跑一次”，还是“团队里任何人都敢长期用”。
...
### 6.2 Frontmatter 只写真正参与路由和门禁的字段
...
## 7. 小结：Skill 是 OpenClaw 的“最小可复用定制单元”

写到这里，你应该能看到 Skill 这层设计的巧妙之处了。

它不是一个重型插件系统，不需要你注册一堆生命周期钩子；它也不是单纯的提示词模板，因为它还能挂脚本、做依赖门禁、接命令分发、走热更新。它处在一个很有现实感的位置：**足够轻，足够工程化，也足够适合团队沉淀自己的领域能力**。

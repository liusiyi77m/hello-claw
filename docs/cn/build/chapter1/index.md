...
> **核心问题**：Agent Runtime 和 Chatbot 的本质区别是什么？为什么 OpenClaw 选择自托管和核心基础工具？
...
本地部署让 OpenClaw 具备了更强的**离线韧性**。拔掉网线，它依然可以：

- 整理本地文件、运行脚本，完成不依赖联网服务的本地工作
- 执行预设的定时任务（如本地备份）
- 维护本地状态（如会话记录、记忆文件等）

但需要调用云端模型或联网工具的步骤，会在离线时失败或等待；网络恢复后再继续执行这些步骤。
...
如果你想从外部访问（比如用手机连接家里的 OpenClaw），需要自己去做一些网络上的配置，这显然会比云服务的"开箱即用"要麻烦。
...
**下一步**：理解了设计理念后，我们将在[第二章 整体架构解析](/cn/build/chapter2/)中，深入了解 OpenClaw 的内部组件是如何协同工作的。
